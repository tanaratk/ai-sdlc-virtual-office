"""Change Impact Agent — Pipeline Step 8.
...
a precise Change Impact Report — what breaks, what must be updated, effort,
...
- Effort estimates must be conservative — multiply first estimate by 1.3.
...
- If a document was not provided, note it as "Not analysed — document not provided".
- Do NOT modify or rewrite upstream documents — analysis only.
...
provided downstream documents and return ONLY the JSON — no explanation.
...
def _render_report(
    ...
    if output.has_breaking_api_change:
        alerts.append("> ⚠️ธ **Breaking API Change detected** — clients must be updated.")
    if output.has_db_migration:
        alerts.append("> ⚠️ธ **Database migration required** — migration script must be prepared.")
    alert_block = "\n".join(alerts) + "\n" if alerts else ""
    ...
    def run(
        ...
        try:
            ...
            report_doc = Document(
                ...
                title=f"Change Impact Report — {', '.join(changed_requirement_ids)}",
