"""Gap Analysis Agent — Pipeline Step 2.
...
Your job is to analyze structured requirement summaries and identify gaps — \
...
def _render_markdown(
    ...
    for gap in sorted_gaps:
        ref = f" ← {gap.related_requirement_id}" if gap.related_requirement_id else ""
        gap_details += f"""
...
class GapAnalysisAgentRunner:
    ...
    def run(self, run_id: uuid.UUID) -> None:
        ...
        try:
            ...
            if run.status == PipelineRunStatus.failed:
                logger.info("GapAnalysisAgent skipped — run %s already failed", run_id)
                return
            ...
            # Gate 1 — wait for human review before proceeding to BA Agent
    ...
    def _load_requirement_summary(self, run_id: uuid.UUID) -> Document:
        ...
        if not req_step or not req_step.output_document_id:
            raise ValueError("requirement_summary step has no output document — cannot run gap analysis")
