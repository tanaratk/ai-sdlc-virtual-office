...
Rules:
- Every component must have a unique ID (COMP-001, COMP-002, โ€ฆ).
- Every database table must have a unique ID (DB-001, DB-002, โ€ฆ).
- Every API endpoint must have a unique ID (API-001, API-002, โ€ฆ).
...
def _render_architecture(data: ArchitectureOutput, project_id: str, doc_id: str) -> str:
    ...
    for c in data.components:
        refs = ", ".join(c.requirement_refs) if c.requirement_refs else "โ€”"
        resp = "\n".join(f"- {r}" for r in c.responsibilities) if c.responsibilities else "> Not specified."
        comp_section += f"""
### {c.id} โ€” {c.name} ({c.type})
...
"""
    ...
    if not int_rows:
        int_rows = "| โ€” | โ€” | No integrations defined |\n"
...
def _render_database(data: DatabaseOutput, project_id: str, doc_id: str) -> str:
    ...
    for t in data.tables:
        ...
        if not col_rows:
            col_rows = "| โ€” | โ€” | โ€” | No columns defined |\n"
        ref = f" โ {t.requirement_ref}" if t.requirement_ref else ""
        tables_section += f"""
### {t.id}{ref} โ€” `{t.name}`
...
"""
    ...
    if not rel_rows:
        rel_rows = "| โ€” | โ€” | โ€” | No relationships defined |\n"
...
def _render_api_spec(data: APISpecOutput, project_id: str, doc_id: str) -> str:
    ...
    for ep in data.endpoints:
        ...
        ref = f" โ {ep.requirement_ref}" if ep.requirement_ref else ""
        ...
        endpoints_section += f"""
### {ep.id}{ref} โ€” `{ep.method} {data.base_url}{ep.path}`
    ...
    def run(self, run_id: uuid.UUID) -> None:
        ...
        try:
            ...
            if run.status == PipelineRunStatus.failed:
                logger.info("SAAgent skipped โ€” run %s already failed", run_id)
                return
            ...
            # Gate 3 โ€” wait for human review of Architecture/DB/API
