    ...
    <div className="space-y-6">
      ...
      {data && data.items.length === 0 && (
        <div className="rounded-lg border border-dashed p-10 text-center">
          ...
          <p className="text-xs text-muted-foreground">Create a project to begin the SDLC pipeline.</p>
        </div>
      )}
            ...
            <li key={project.id}>
              ...
              >
                <div className="flex items-start justify-between gap-2">
                  ...
                  <span className={`rounded-full px-2 py-0.5 text-xs font-medium flex-shrink-0 ${STATUS_COLORS[project.status]}`}>
                    {project.status}
                  </span>
                </div>
                {project.description && (
